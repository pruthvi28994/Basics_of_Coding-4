...
import java_Examples.java_SearchingTech;

public class java_AllSort {
	
	//calling the class seaching tech so that we can reuse the array and variables instead of redefining it
		...
		for(int i=0;i<search.MAX_SIZE-1;i++) {
			for(int j=0;j<search.MAX_SIZE-i-1;j++) {
				if(search.search_array[j]>search.search_array[j+1]) {
					//in java there is no concept called pointer hence iam passing the position value
					search.swap(j,j+1);
				}
			}
		}
